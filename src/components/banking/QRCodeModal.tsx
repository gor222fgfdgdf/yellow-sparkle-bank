...
import { QrCode, Copy, Check, Scan, Share2 } from "lucide-react";
...
const QRCodeModal = ({ isOpen, onClose, userName, cardNumber, onReceive }: QRCodeModalProps) => {
  ...
  return (
    <FullScreenModal isOpen={isOpen} onClose={onClose} title="QR-код для платежей">
      {/* Mode Toggle */}
      ...
      {mode === "show" ? (
        <div className="space-y-6">
          {/* QR Code Display */}
          <div className="bg-card p-6 rounded-2xl flex flex-col items-center border border-border">
        ...
        <div className="space-y-6">
          {/* Scanner View */}
          <div className="bg-muted rounded-2xl aspect-square flex flex-col items-center justify-center relative overflow-hidden">
            {scanning ? (
              <>
