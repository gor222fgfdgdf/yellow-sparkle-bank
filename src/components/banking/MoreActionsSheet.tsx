...
import { FileText, Settings, Bell, Shield, Gift, Download, Share2, Check, CreditCard, PiggyBank, Calculator, Calendar, Lock, Repeat } from "lucide-react";
...
const MoreActionsSheet = ({ isOpen, onClose, onOpenSubscriptions, onOpenStatementExport, onOpenAccountCertificate, onOpenBudgets, onOpenSavingsGoals, onOpenLoans, onOpenInsurance, onOpenCalendar, onSetupPin }: MoreActionsSheetProps) => {
  ...
  // Sub-sections rendered as separate FullScreenModals
  if (activeSection === "statements") {
    ...
    return (
      <FullScreenModal isOpen={isOpen} onClose={() => { setActiveSection(null); onClose(); }} title="Выписки по счёту">
        <div className="space-y-3">
          {statements.map((statement) => (
            <button key={statement.month} onClick={() => toast({ title: "Загрузка", description: `Выписка за ${statement.month} загружается...` })} className="w-full flex items-center justify-between p-4 bg-muted rounded-xl hover:bg-muted/80 transition-colors">
  ...
  if (activeSection === "notifications") {
    return (
      <FullScreenModal isOpen={isOpen} onClose={() => { setActiveSection(null); onClose(); }} title="Настройки уведомлений">
  ...
  if (activeSection === "referral") {
    return (
      <FullScreenModal isOpen={isOpen} onClose={() => { setActiveSection(null); onClose(); }} title="Приведи друга">
  ...
  if (activeSection === "security") {
    return (
      <FullScreenModal isOpen={isOpen} onClose={() => { setActiveSection(null); onClose(); }} title="Безопасность">
        <div className="space-y-3">
  ...
  if (activeSection === "settings") {
    return (
      <FullScreenModal isOpen={isOpen} onClose={() => { setActiveSection(null); onClose(); }} title="Настройки">
        <div className="space-y-3">
          {[
            { label: "Язык", value: "Русский" },
            { label: "Валюта", value: "RUB (₽)" },
            { label: "Версия", value: "2.5.0" },
          ].map(({ label, value }) => (
  ...
  return (
    <FullScreenModal isOpen={isOpen} onClose={onClose} title="Ещё">
