...
const PaymentTemplatesModal = ({ isOpen, onClose, onPayment }: PaymentTemplatesModalProps) => {
  ...
  return (
    <FullScreenModal isOpen={isOpen} onClose={onClose} title="Шаблоны платежей">
      {templates.length > 0 && !isAddingNew && (
        <div className="space-y-3 mb-6">
          {templates.map((template) => (
            <div key={template.id} className="p-4 bg-muted/50 rounded-xl flex items-center justify-between">
