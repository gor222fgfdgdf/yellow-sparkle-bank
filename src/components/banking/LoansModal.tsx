...
const loanOffers: LoanOffer[] = [
  {
    ...
    features: ["Без справок", "Решение за 2 минуты", "Досрочное погашение"]
  },
  {
    id: "2",
    ...
    maxAmount: 3000000,
    maxTerm: 48,
    description: "Объедините кредиты со снижением ставки",
    features: ["Ставка ниже", "Один платёж", "Снижение переплаты"]
  },
  {
    id: "3",
    name: "Рассрочка",
    ...
    maxAmount: 500000,
    maxTerm: 24,
    description: "0% на покупки у партнёров",
    features: ["Без переплаты", "5000+ магазинов", "Мгновенное одобрение"]
  }
];
...
const activeLoans: ActiveLoan[] = [
  {
    ...
    monthlyPayment: 15420,
    nextPaymentDate: "25 дек",
    rate: 14.9
  }
];
    ...
    <Sheet open={isOpen} onOpenChange={onClose}>
      <SheetContent side="bottom" className="h-[90vh] rounded-t-3xl">
        ...
        <Tabs defaultValue="offers" className="h-[calc(90vh-100px)]">
          ...
          <TabsContent value="offers" className="space-y-4 overflow-y-auto max-h-[calc(90vh-180px)]">
            {selectedOffer ? (
              <div className="space-y-6">
                <button onClick={() => setSelectedOffer(null)} className="text-primary text-sm">
                ...
                <div className="space-y-4">
                  <div>
                    <Label>Сумма: {amount.toLocaleString("ru-RU")} ₽</Label>
                    <Slider
                      value={[amount]}
                  ...
                  <div className="bg-muted rounded-xl p-4">
                    ...
                    <p className="text-2xl font-bold text-foreground">
                      {Math.round(calculateMonthlyPayment(amount, selectedOffer.rate, term)).toLocaleString("ru-RU")} ₽
                    </p>
                  </div>

                  <Button onClick={handleApply} className="w-full" disabled={isApplying}>
                    {isApplying ? "Оформление..." : "Оформить кредит"}
                  </Button>
                </div>
              </div>
            ) : (
            ...
            <div className="bg-card rounded-2xl p-4 border border-border space-y-4">
              ...
              <div className="flex justify-between">
                ...
                <span className="font-bold text-foreground text-lg">
                  {Math.round(calculateMonthlyPayment(amount, 14.9, term)).toLocaleString("ru-RU")} ₽
                </span>
              </div>
              <div className="flex justify-between">
                <span className="text-muted-foreground">Переплата</span>
                <span className="font-medium text-foreground">
                  {Math.round(calculateMonthlyPayment(amount, 14.9, term) * term - amount).toLocaleString("ru-RU")} ₽
                </span>
              </div>
              <div className="flex justify-between">
                <span className="text-muted-foreground">Общая сумма</span>
                <span className="font-medium text-foreground">
                  {Math.round(calculateMonthlyPayment(amount, 14.9, term) * term).toLocaleString("ru-RU")} ₽
                </span>
              </div>
            </div>
          ...
          <TabsContent value="active" className="space-y-4 overflow-y-auto max-h-[calc(90vh-180px)]">
            {activeLoans.length > 0 ? (
                ...
                <div key={loan.id} className="bg-card rounded-2xl p-4 border border-border space-y-4">
                  <div className="flex items-center justify-between">
                    <div>
                      ...
                      <p className="text-sm text-muted-foreground">{loan.rate}% годовых</p>
                    </div>
                  ...
                  <div>
                    ...
                    <Progress value={(1 - loan.remainingAmount / loan.totalAmount) * 100} />
                  </div>

                  <div className="grid grid-cols-2 gap-4">
                    <div>
                      <p className="text-sm text-muted-foreground">Остаток</p>
                      <p className="font-bold text-foreground">{loan.remainingAmount.toLocaleString("ru-RU")} ₽</p>
                    </div>
                    <div>
                      <p className="text-sm text-muted-foreground">Платёж</p>
                      <p className="font-bold text-foreground">{loan.monthlyPayment.toLocaleString("ru-RU")} ₽</p>
                    </div>
                  </div>
                  ...
                  <div className="flex items-center gap-2 text-sm">
                    ...
                    <span className="font-medium text-foreground">{loan.nextPaymentDate}</span>
                  </div>

                  <div className="flex gap-2">
                    <Button variant="outline" className="flex-1">График</Button>
                    <Button className="flex-1">Погасить</Button>
                  </div>
                </div>
              ))
            ) : (
              <div className="text-center py-12">
                ...
                <h3 className="font-semibold text-foreground mb-2">Нет активных кредитов</h3>
                <p className="text-muted-foreground">Оформите выгодный кредит прямо сейчас</p>
              </div>
            )}
          </TabsContent>
        </Tabs>
