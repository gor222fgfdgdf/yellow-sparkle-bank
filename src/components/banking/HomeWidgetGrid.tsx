...
const HomeWidgetGrid = ({ totalBalance, onQRCode, onReferral, onCashback, onAnalytics }: HomeWidgetGridProps) => {
  return (
    <div className="grid grid-cols-4 gap-2 auto-rows-auto">
      {/* QR Code - small square */}
      <button onClick={onQRCode} className="bg-card rounded-2xl p-3 flex items-center justify-center aspect-square">
        <QrCode className="w-7 h-7 text-foreground" />
      </button>

      {/* UnionPay logo - small square */}
      <button className="bg-card rounded-2xl p-3 flex items-center justify-center aspect-square">
        <UnionPayLogo className="w-8 h-8" />
      </button>

      {/* Доступно баллов - spans 2 cols */}
      <button onClick={onCashback} className="bg-card rounded-2xl p-3 col-span-2 flex flex-col items-start justify-between relative overflow-hidden">
        <p className="text-xs text-muted-foreground font-medium">Доступно</p>
      ...
      {/* Приведи друга - spans 2 cols, 1 row */}
      <button onClick={onReferral} className="bg-primary rounded-2xl p-3 col-span-2 flex items-center gap-2">
        <Users className="w-5 h-5 text-primary-foreground" />
        <span className="text-sm font-medium text-primary-foreground">Приведи друга</span>
      </button>

      {/* Анализ финансов - spans 2 cols */}
      <button onClick={onAnalytics} className="bg-card rounded-2xl p-3 col-span-2 flex flex-col items-start">
        <p className="text-xs text-muted-foreground font-medium">Анализ финансов</p>
