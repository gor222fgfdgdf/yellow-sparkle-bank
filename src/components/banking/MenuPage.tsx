...
const MenuPage = ({ onOpenCardManagement }: MenuPageProps) => {
  ...
  // Info tab list items
  const infoItems = [
    { icon: QrCode, label: "Вход / Операция в Интернет-банке", subtitle: "Сканировать QR-код", highlight: true },
    ...
    { icon: FileText, label: "Документы", subtitle: "На подпись и подписанные" },
    { icon: Headphones, label: "Связь с банком", subtitle: "Телефоны для связи", action: () => setIsSupportOpen(true) },
    { icon: Info, label: "Вопросы и ответы по системе", subtitle: "Лимиты, тарифы и другая информация" },
    { icon: Newspaper, label: "Новости", subtitle: "Новое в приложении" },
  ];

  // Products - main product cards
  const productCards = [
  ...
  const productGrid = [
    ...
    { label: "Подключить подписку Всё Своё" },
    { label: "Купить монеты и металлы", action: () => setIsCurrencyOpen(true) },
  ];

  const otherOffers = [
    { icon: Briefcase, label: "Всё для бизнеса" },
    { icon: UserCheck, label: "Всё для самозанятых" },
    { icon: Plane, label: "Рассказы о путешествиях" },
  ];

  // Settings items
  const settingsItems = [
    { icon: Phone, label: "Переводы по номеру телефона", subtitle: "Настройте систему быстрых переводов" },
    { icon: Bell, label: "Push-уведомления", subtitle: "Настройте Push уведомления", toggle: true },
    { icon: MessageSquare, label: "Оповещения", subtitle: "По почте и по телефону" },
    { icon: LayoutGrid, label: "Виджет", subtitle: "Настройка виджетов" },
    { icon: Gauge, label: "Лимиты", subtitle: "Настройка лимитов по операциям", action: () => setIsLimitsOpen(true) },
    { icon: ShieldIcon, label: "Безопасность", subtitle: "Настройки входа и безопасности" },
    { icon: Smartphone, label: "Мои устройства", subtitle: "Добавьте используемое устройство" },
    { icon: CreditCard, label: "Лимиты на покупки через СБП", subtitle: "Установить лимиты на покупки в магазинах" },
  ];
    ...
    <div className="pb-24">
      ...
      <div className="flex items-center gap-3 px-4 pt-2 pb-4">
        ...
        <button className="flex items-center gap-1">
          <span className="text-lg font-bold text-foreground">Котиков Егор</span>
            ...
            {infoItems.map((item) => (
              ...
              >
                <div className={`w-12 h-12 rounded-2xl flex items-center justify-center shrink-0 ${
                  item.highlight ? "bg-primary/10" : "bg-primary/10"
                }`}>
      ...
      {activeTab === "products" && (
        <div className="space-y-0">
          ...
          <div className="px-4 pt-6">
            ...
            {/* Credit card promo */}
            <button className="w-full bg-primary rounded-2xl p-5 mb-3 text-left hover:bg-primary/90 transition-colors">
      ...
      <SpendingLimitsModal open={isLimitsOpen} onOpenChange={setIsLimitsOpen} limits={[]} onSaveLimits={() => {}} categories={["Продукты", "Транспорт", "Развлечения", "Рестораны"]} />
      {isSupportOpen && (
      ...
      <CurrencyExchangeModal
        ...
        balance={30327.84}
        onExchange={(amount, from, to) => {}}
      />
      <InvestmentPortfolioModal isOpen={isInvestOpen} onClose={() => setIsInvestOpen(false)} portfolioValue={0} />
    </div>
  );
};
