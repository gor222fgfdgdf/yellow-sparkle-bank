...
BEGIN
    ...
    INSERT INTO public.accounts (user_id, type, name, balance, card_number, account_number, color)
    VALUES (NEW.id, 'card', 'UnionPay Classic QuickPass', 0, '4276', '40817810514230007456', 'bg-foreground text-background')
    RETURNING id INTO main_account_id;

    INSERT INTO public.accounts (user_id, type, name, balance, card_number, account_number, color)
    VALUES (NEW.id, 'card', 'Классическая карта МИР', 0, '6030', '40817810514230008912', 'bg-primary text-primary-foreground');

    INSERT INTO public.accounts (user_id, type, name, balance, color)
    VALUES (NEW.id, 'investment', 'Инвестиции', 0, 'bg-primary text-primary-foreground')
    ...
    VALUES (NEW.id, 'credit', 'Кредитная карта', 0, '5213', 300000.00, 4500.00, CURRENT_DATE + INTERVAL '15 days', 'bg-destructive text-destructive-foreground')
